...
public partial class TelnetInterpreter
{
    ...
    private StateMachine<State, Trigger> SetupTelnetTerminalTypeAsClient(StateMachine<State, Trigger> tsm)
    {
        ...
        tsm.Configure(State.DontTType)
            .SubstateOf(State.Accepting)
            .OnEntry(() => _logger.LogDebug("Connection: {ConnectionState}", "Server telling us not to Terminal Type"));
    ...
    private StateMachine<State, Trigger> SetupTelnetTerminalTypeAsServer(StateMachine<State, Trigger> tsm)
    {
        ...
        tsm.Configure(State.WontDoTType)
            .SubstateOf(State.Accepting)
            .OnEntry(() => _logger.LogDebug("Connection: {ConnectionState}", "Client won't do Terminal Type"));
        ...
        TriggerHelper.ForAllTriggersButIAC(t =>
            tsm.Configure(State.NegotiatingTerminalType).Permit(t, State.EvaluatingTerminalType));
        TriggerHelper.ForAllTriggers(t =>
            tsm.Configure(State.EvaluatingTerminalType).OnEntryFrom(ParameterizedTrigger(t), CaptureTerminalType));
    ...
    private async ValueTask CompleteTerminalTypeAsServerAsync()
    {
        ...
        {
            _logger.LogTrace("Connection: {ConnectionState}: {TerminalType}",
