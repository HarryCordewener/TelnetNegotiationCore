...
- **Reporting a window size has moved to the NAWS plugin and now covers RFC 1073's full range: `TelnetInterpreter.SendNAWS(short, short)` is gone, replaced by `NAWSProtocol.SendWindowSizeAsync(int, int)`.** RFC 1073 defines both dimensions as 16-bit **unsigned** fields, and says so as the reason the option exists at all — *"the 253 character height and width limitation is too low so the new option has a limit of 65535 characters"*. `short` reaches 32767, so the top half of that range was only expressible by passing a negative two's-complement bit pattern: `SendNAWS(-1, -1)` really did put 65535 on the wire, and nothing said so. 2.7.0 widened the **receive** path to the full unsigned range, so the library read 0–65535 and wrote 0–32767; that asymmetry is now closed. Everything else about NAWS — the callback, `ClientWidth`/`ClientHeight`, the `DO NAWS` gate, the escaping — is unchanged, and the widened sender round-trips through the existing reader, including 65535 × 65535, whose eight `0xFF` payload bytes are all doubled as the same RFC requires.
  ...
  - **Three unreachable copies of the NAWS receive path are removed, one of them public.** `Interpreters/TelnetNAWSInterpreter.cs` held a private `CompleteNAWSAsync` with its own capture buffer, wired into no state machine and kept alive only by a `#pragma warning disable CS0414`, and `NAWSProtocol.OnNAWSNegotiatedAsync` (internal) existed solely for it to call. `NAWSProtocol` itself held a third: the **public** `ProcessNAWSByte(byte)` and `CompleteNAWSNegotiationAsync()`, which no code in the library, the tests, TestClient or TestServer ever called, and which read the width and height back with `+` rather than the live path's `|`. Their only consumer hook was a `NAWS_Callback` shared-state key that nothing in the repository ever writes — a callback that could not have fired even if a consumer had set it, since the method that reads it was unreachable. Window size has arrived through `.OnNAWS(...)` and the state machine's own `CaptureNAWS`/`CompleteNAWSAsync` throughout. **Removing the two public members is a further binary break** on top of `SendNAWS`; anything that somehow bound to them gets `MissingMethodException`, and there is nothing to migrate to because the path never ran.
